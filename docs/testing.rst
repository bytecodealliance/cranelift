...
:file:`src/cranelift-codegen` and :file:`src/cranelift-reader` crates have no external
dependencies, which can make testing tedious. Integration tests that don't need
to depend on other crates can be placed in :file:`src/cranelift-codegen/tests` and
:file:`src/cranelift-reader/tests`.
...
:file:`src/cranelift-codegen/meta-python/isa/*/settings.py`.
...
:file:`src/cranelift-codegen/meta-python/base/settings.py`.
