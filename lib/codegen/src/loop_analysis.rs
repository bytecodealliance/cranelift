...
use ir::{Ebb, Function, InstBuilder, Layout, ProgramOrder};
...
impl LoopData {
    ...
    fn new(header: Ebb, parent: Option<Loop>) -> Self {
        Self {
            header,
            parent: parent.into(),
        }
...
impl LoopAnalysis {
    ...
    fn discover_loop_blocks(
        ...
        let mut global_side_effects = SideEffects {
            ebb_splitted: false,
        };
            ...
            None => {
                ...
                self.ebb_loop_map[ebb] = EbbLoopData {
                    ...
                    last_inst: func.layout
                        .last_inst(ebb)
                        .map_or(None.into(), |ebb_last_inst| {
                            if ebb_last_inst != loop_edge_inst {
                                loop_edge_inst.into()
                            } else {
                                None.into()
                            }
                        ...
                        if inner_loop == lp {
                                ...
                                Some(previous_last_inst) => {
                                        ...
                                        == Ordering::Greater
                                    {
                                        if loop_edge_inst == func.layout.last_inst(ebb).unwrap() {
                                            ...
                                            self.ebb_loop_map[ebb].last_inst =
                                                loop_edge_inst.into();
                                        }
                                    }
                                }
                            ...
                            match self.ebb_loop_map[ebb].last_inst.expand() {
                                ...
                                Some(last_inner_loop) => {
                                    ...
                                    if func.layout
                                        .last_inst(ebb)
                                        .map_or(false, |ebb_last_inst| ebb_last_inst != limit)
                                    {
                                        // This handles the second case
                                        self.split_ebb_containing_two_loops(
                                            ebb, limit, lp, func, domtree, cfg,
                                        );
                                        split_ebb = true;
                                    }
                                }
...
impl LoopAnalysis {
    ...
    fn split_ebb_containing_two_loops(
        &mut self,
        ebb: Ebb,
        split_inst: Inst,
        lp: Loop,
        ...
        let middle_jump_inst = {
            let cur = &mut FuncCursor::new(func);
            cur.goto_bottom(ebb);
            cur.ins().jump(new_ebb, &[])
        };
...
mod test {
    ...
    use dominator_tree::DominatorTree;
    use flowgraph::ControlFlowGraph;
    use ir::{types, Ebb, Function, InstBuilder};
    ...
    fn ebb_splitting() {
        ...
        let (inst0, inst1) = {
            ...
            cur.insert_ebb(ebb1);
            let inst0 = cur.ins().brnz(cond, ebb1, &[]);
            let inst1 = cur.ins().brnz(cond, ebb0, &[]);
            cur.ins().jump(ebb2, &[]);
        ...
        assert_eq!(
            loop_analysis.is_in_loop(Ebb::with_number(3).unwrap(), loops[0]),
            true
        );
        assert_eq!(
            loop_analysis
    ...
    fn ebb_splitting_complex() {
        ...
        let inst1 = {
            ...
            cur.insert_ebb(ebb1);
            cur.ins().brnz(cond, ebb2, &[]);
            cur.ins().jump(ebb1, &[]);

            cur.insert_ebb(ebb2);
            let inst1 = cur.ins().jump(ebb0, &[]);
            cur.ins().brnz(cond, ebb3, &[]);
        ...
        assert_eq!(
            loop_analysis.last_loop_instruction(ebb1, loops[1]).unwrap(),
            None
        );
        assert_eq!(
            loop_analysis
