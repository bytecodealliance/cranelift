...
pub fn expand_template(
    ...
    match func.templates[template] {
        ...
        ir::TemplateData::Symbol { .. } => symbol(inst, func, template, isa),
...
/// Expand a `template` instruction for a symbolic name.
fn symbol(inst: ir::Inst, func: &mut ir::Function, template: ir::Template, isa: &dyn TargetIsa) {
    let ptr_ty = isa.pointer_type();
