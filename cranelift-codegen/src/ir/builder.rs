...
use crate::ir::{Function, Inst, Opcode, Type, Value};
use crate::isa;
...
pub trait InstBuilderBase<'f>: Sized {
    ...
    fn build(self, data: InstructionData, ctrl_typevar: Type) -> (Inst, &'f mut Function);
}
...
pub trait InstInserterBase<'f>: Sized {
    ...
    fn insert_built_inst(self, inst: Inst, ctrl_typevar: Type) -> &'f mut Function;
}
...
impl<'f, IIB: InstInserterBase<'f>> InstBuilderBase<'f> for InsertBuilder<'f, IIB> {
    ...
    fn function_mut(&mut self) -> &mut Function {
        self.inserter.function_mut()
    }

    fn build(mut self, data: InstructionData, ctrl_typevar: Type) -> (Inst, &'f mut Function) {
        let inst;
        {
            let func = self.inserter.function_mut();
            inst = func.dfg.make_inst(data);
            func.dfg.make_inst_results(inst, ctrl_typevar);
        }
    ...
    Array: AsRef<[Option<Value>]>,
{
    ...
    fn build(mut self, data: InstructionData, ctrl_typevar: Type) -> (Inst, &'f mut Function) {
        let inst;
        {
            ...
            func.dfg.make_inst_results_reusing(inst, ctrl_typevar, ru);
        }
        (inst, self.inserter.insert_built_inst(inst, ctrl_typevar))
    }
}
...
impl<'f> ReplaceBuilder<'f> {
    ...
    pub fn new(func: &'f mut Function, inst: Inst) -> Self {
        Self { func, inst }
    }
}
...
impl<'f> InstBuilderBase<'f> for ReplaceBuilder<'f> {
    ...
    fn build(self, data: InstructionData, ctrl_typevar: Type) -> (Inst, &'f mut Function) {
        ...
        self.func.dfg[self.inst] = data;

        if !self.func.dfg.has_results(self.inst) {
            // The old result values were either detached or non-existent.
            // Construct new ones.
            self.func.dfg.make_inst_results(self.inst, ctrl_typevar);
        }
