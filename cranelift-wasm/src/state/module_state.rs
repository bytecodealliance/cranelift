...
use crate::translation_utils::{get_vmctx_value_label, SignatureIndex};
use crate::{wasm_unsupported, FuncEnvironment, FuncTranslationState, ReturnMode, WasmResult};
...
impl ModuleTranslationState {
    ...
    ///
    pub fn translate_func<FE: FuncEnvironment + ?Sized>(
        &self,
        code: &[u8],
        code_offset: usize,
        ...
        self.translate_func_with_reader(
    ...
    /// Translate a binary WebAssembly function with a `BinaryReader` rather
    /// than `[u8]` slice.
    pub fn translate_func_with_reader<FE: FuncEnvironment + ?Sized>(
        &self,
        ...
        state.initialize(&builder.func.signature, exit_block);

        parse_local_decls(&mut reader, &mut builder, num_params, environ)?;
        parse_function_body(self, reader, &mut builder, &mut state, environ)?;
...
mod tests {
    ...
    fn small1() {
        ...
        module_translation_state
            .translate_func(&BODY, 0, &mut ctx.func, &mut runtime.func_env())
            .unwrap();
