    ...
    while let Some(_ebb) = pos.next_ebb() {
        while let Some(inst) = pos.next_inst() {
            ...
            match pos.func.dfg[inst] {
                Binary { opcode, args } => {
                    fold_binary(&mut pos.func, inst, opcode, args);
                }
                Unary { opcode, arg } => {
                    fold_unary(&mut pos.func, inst, opcode, arg);
                }
                Branch {
                    opcode,
                    args: _,
...
fn resolve_value_to_imm(func: &ir::Function, value: ir::Value) -> Option<ConstImm> {
    let original = func.dfg.resolve_aliases(value);

    let inst = func.dfg.value_def(original).unwrap_inst();
...
fn replace_inst(func: &mut ir::Function, inst: ir::Inst, const_imm: ConstImm) {
    use self::ConstImm::*;
    match const_imm {
        I64(imm) => {
            let typevar = func.dfg.ctrl_typevar(inst);
            func.replace(inst).iconst(typevar, imm);
        }
        Ieee32(imm) => {
            func.replace(inst)
        ...
        Bool(imm) => {
            let typevar = func.dfg.ctrl_typevar(inst);
            func.replace(inst).bconst(typevar, imm);
        }
    }
}

/// Fold a binary instruction.
fn fold_binary(func: &mut ir::Function, inst: ir::Inst, opcode: ir::Opcode, args: [ir::Value; 2]) {
    let (imm0, imm1) = if let (Some(imm0), Some(imm1)) = (
        resolve_value_to_imm(func, args[0]),
        resolve_value_to_imm(func, args[1]),
    ) {
        (imm0, imm1)
    } else {
        return;
    ...
    if let Some(const_imm) = evaluate_binary(opcode, imm0, imm1) {
        replace_inst(func, inst, const_imm);
    }
}

/// Fold a unary instruction.
fn fold_unary(func: &mut ir::Function, inst: ir::Inst, opcode: ir::Opcode, arg: ir::Value) {
    let imm = if let Some(imm) = resolve_value_to_imm(func, arg) {
        imm
    } else {
        return;
    };
...
fn fold_branch(pos: &mut FuncCursor, inst: ir::Inst, opcode: ir::Opcode) {
    let (cond, ebb, args) = {
        ...
        (
            resolve_value_to_imm(&pos.func, values[0]).unwrap(),
    ...
    if (branch_if_zero && !truthiness) || (!branch_if_zero && truthiness) {
        pos.func.replace(inst).jump(ebb, &args);
